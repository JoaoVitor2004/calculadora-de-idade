...
botao.addEventListener('click', () => {
    ...
    if (txtDia.value <= 0 || txtDia.value > 31 || txtMes.value <= 0 || txtMes.value > 12 || txtAno.value <= 0 || txtAno.value > ano ) {
        ...
        const etiquetas = [label1, label2, label3].forEach((etiquetas) => etiquetas.style = 'color: red;')
        const erros = [erro1, erro2, erro3].forEach((erros) => {
            erros.classList.remove('hide')
            erros.style.color = 'red'
        })
        
    } else {
        const etiquetas = [label1, label2, label3].forEach((etiquetas) => etiquetas.style = 'color: black;')
        const erros = [erro1, erro2, erro3].forEach((erros) => erros.classList.add('hide'))
        ...
        const subtrairDia = dia - converterDia 

        res1.innerHTML = subtrairAno - 1
